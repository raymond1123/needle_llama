...
public:
    __device__ void operator()(size_t n,
        ...
        if constexpr (std::is_same<Dtype, __half>::value) {
            out[tx] = __float2half(__half2float(b[tx])*
                        (1-pow(tanh(__half2float(a[tx])), 2)));
        } else 
            out[tx] = b[tx]*(1-pow(tanh(a[tx]), 2));
    }
};

template<typename Dtype>
__global__ void __launch_bounds__(kBlockSize)
ApplyTanh(size_t n, const Dtype* a, Dtype* out) {
    auto functor = Tanh<Dtype>();
    functor(n, a, out);
}

template<typename Dtype>
__global__ void __launch_bounds__(kBlockSize)
ApplyTanhGrad(size_t n, const Dtype* a, const Dtype* b, Dtype* out) {
    auto functor = TanhGrad<Dtype>();
    functor(n, a, b, out);
}

template<typename Dtype>
class TanhOp: public GenericOp<Dtype> {
protected:
    using cached_data_type = std::shared_ptr<BaseTensor<Dtype>>;

public:
    TanhOp(OpType op_type): GenericOp<Dtype>(op_type) {}

    virtual cached_data_type compute(std::vector<cached_data_type> inputs) override {

        assert(inputs.size()==1 && "input number of TanhOp must be 1");
        ...
        assert(err==cudaSuccess && "get_num_blocks in TanhOp failed");

        ApplyTanh<Dtype><<<_num_blocks, kBlockSize, 0>>>(_n,
                                                  inputs[0]->cached_ptr(),
                                                  cached_data->cached_ptr());

        err = cudaPeekAtLastError();
        assert(err==cudaSuccess && "ApplyTanh failed");
    ...
    virtual std::vector<cached_data_type> gradient(cached_data_type out_grad, 
        ...
        ApplyTanhGrad<Dtype><<<_num_blocks, kBlockSize, 0>>>(_n,
                                                  inputs[0]->cached_ptr(),
                                                  out_grad->cached_ptr(),
