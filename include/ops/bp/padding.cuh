...
public:
    PaddingOp(std::vector<int> axes, OpType op_type): 
...
private:
    void calc_new_shape() {
        ...
        for(int i=_axes.size()-2; i>=0; i-=2)
        ...
        for(int i=0; i<_org_shape.size(); ++i) {
            int axes_idx = (_org_shape.size()-1-i)*2;
            _slices.push_back(py::slice(_axes[axes_idx], _axes[axes_idx]+_org_shape[i], 1));
        }
    }
