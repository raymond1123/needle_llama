...
if __name__ == '__main__':
    ...
    model = LLaMA.build(
        checkpoints_dir=llama_model,
        tokenizer_path=f'{llama_model}/tokenizer.model',
        load_model=True,
        max_seq_len=1024,
        max_batch_size=len(prompts2),
        device=device
    )

    out_tokens, out_texts = (model.text_completion(prompts2, max_gen_len=1024))
    assert len(out_texts) == len(prompts2)
